...
int TC02_Basic_MultiOutput()
{
    ...
    if (gs == NULL)
    {
        printf("Create logging group handle failed. Errno:[%d]\n", errno);
        return -1;
    }
    else
    {
        printf("Create logging group handle successful.\n");
    }
    ...
    if (g == NULL)
    {
        printf("Create logging handle failed. Errno:[%d]\n", errno);
        return -1;
    }
    else
    {
        printf("Create logging handle successful.\n");
    }
    /* 设置日志输出文件名 */
    ...
    printf("Released the logging group handle.\n");
